...
const chapterPlan = [
  ...
  },
  {
    ...
    subchapters: [
      section(5, 1, 'Gener ger oss våra egenskaper', 266, 277, 'section', 'indexed_with_boundary_conflict', 'medium'),
      section(5, 2, 'Celldelning och mutationer', 278, 291, 'section', 'indexed_with_boundary_conflict', 'medium'),
      section(5, 3, 'Hur egenskaper går i arv', 292, 301, 'section', 'indexed_with_boundary_conflict', 'medium'),
      section(5, 4, 'Hur arv och miljö påverkar våra egenskaper', 302, 307, 'section', 'indexed_with_boundary_conflict', 'medium'),
      section(5, 5, 'Avel, växtförädling och genteknik', 308, 317, 'section', 'indexed_with_boundary_conflict', 'medium'),
      section(5, 6, 'Genteknik i vård och forskning', 318, 323, 'section', 'indexed_with_boundary_conflict', 'medium')
    ]
  },
  {
    chapterNo: 6,
    ...
    subchapters: [
      section(6, 1, 'Evolutionen - så fungerar den', 326, 337, 'section', 'indexed_with_boundary_conflict', 'medium'),
      section(6, 2, 'Evidens för evolutionen', 338, 343, 'section', 'indexed_with_boundary_conflict', 'medium'),
      section(6, 3, 'Livets uppkomst och tidiga utveckling', 344, 350, 'section', 'indexed_with_boundary_conflict', 'medium'),
      section(6, 4, 'Det flercelliga livets utveckling', 351, 361, 'section', 'indexed_with_boundary_conflict', 'medium')
    ]
  }
];
...
function sourceIdFor(chapterNo) {
  return sourceByChapter[chapterNo].privateLocator.replace('private-source://pdfer/biologi/', '');
}

function evidenceRefIdsFor(chapterNo, pageSpan) {
  const sourceId = sourceIdFor(chapterNo);
  const ids = [];
  for (let page = pageSpan.start; page <= pageSpan.end; page += 1) {
    ids.push(`evidence-ref-physical-page-${slug(sourceId)}-s${page}`);
  }
  return ids;
}
...
const toc = {
  ...
  sourceSummary: Object.values(sourceByChapter).map((source) => ({
    kind: 'private_source',
    locator: source.privateLocator,
    confidence: source.primarySource === 'Biologi-del-3' || source.primarySource === 'Biologi-del-1 + Biologi-del-2' ? 'medium' : 'high'
  })),
  chapters,
  boundaryDecisions: [
    {
